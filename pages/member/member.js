...
Page({
        ...
        if (images) {
            wx.uploadFile({
                ...
                success: function (res) {
                    ...
                    if (res.status == 200) {
                        that.formRequst(rd_session, sex, province, city, county);
                    } else {
                        wx.showModal({
                            showCancel: true,
                            title: '提示',
    ...
    //表单请求修改性别和地址
    formRequst: function (rd_session, sex, province, city, county) {
        wx.request({
